import 'package:crystal_skin_admin/models/order.dart';
...
class _OrderListScreenState extends State<OrderListScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_selectedStatus != 'Sve'){
        params['Status'] = (_statusOptions.indexOf(_selectedStatus) - 1).toString();
      }
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (order.transactionId?.isNotEmpty == true)
                              _buildDetailRow('Broj transakcije:', order.transactionId ?? 'N/A'),
                            _buildDetailRow('Adresa:', order.address ?? 'N/A'),
                            _buildDetailRow('Telefon:', order.phoneNumber ?? 'N/A'),
                            _buildDetailRow('Datum:', order.date != null ? DateFormat('dd.MM.yyyy HH:mm').format(order.date!) : 'N/A'),
                            _buildDetailRow('Način plaćanja:', order.paymentMethod ?? 'N/A'),
                            _buildDetailRow('Način dostave:', order.deliveryMethod ?? 'N/A'),
                            _buildDetailRow('Status:', _getStatusText(order.status ?? 0)),
                            _buildDetailRow('Ukupno:', '${order.totalAmount.toStringAsFixed(2)} KM'),
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            subtitle: Text('${item.quantity} x ${item.unitPrice.toStringAsFixed(2)} KM'),
                            trailing: Text(
  ...
  String _getStatusText(int status) {
    switch (status) {
      ...
      case 1: return 'Poslano';
      case 2: return 'Isporučeno';
      case 3: return 'Otkazano';
                          ...
                          onPressed: () async {
                            try {
                              ...
                              switch (_selectedChangeStatus) {
                                case 'Na čekanju': newStatus = 0; break;
                                case 'Poslano': newStatus = 1; break;
                                case 'Isporučeno': newStatus = 2; break;
                                case 'Otkazano': newStatus = 3; break;
                                default: newStatus = 0;
                              }

                              await _orderProvider.changeOrderStatus(order.id, newStatus);

                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text("Status narudžbe uspješno ažuriran"),
                                  backgroundColor: Colors.green,
                                  behavior: SnackBarBehavior.floating,
                                  duration: Duration(seconds: 2),
                                ),
